...
- Frontend ligger i app-paketet (ingen remote-URL), vilket är det Apple tittar efter.

## In-App Purchase (TestFlight-checklista)

1. `cordova-plugin-purchase` finns i `package.json` (samma plugin som Mr.
   Battery Doc) och installeras via Capacitors Cordova-lager. Kör:
   ```bash
...
4. Verifiera efter sync att `cordova-plugin-purchase` finns i
   `ios/capacitor-cordova-ios-plugins` och att Xcode-loggen visar
   `CdvPurchase` / `StoreKit` vid appstart.
